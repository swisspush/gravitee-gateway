...
public class HttpClientAdapter extends AbstractHttpClient {
    ...
    public HttpClientAdapter(Reactor reactor) {
        this.reactor = reactor;
    }

    @Override
    protected HttpClientRequest doRequest(HttpMethod httpMethod, String user, Handler<HttpClientResponse> handler) {
        HttpClientRequestAdapter vertxRequest = new HttpClientRequestAdapter();
        HttpClientResponseAdapter response = new HttpClientResponseAdapter(handler);
        reactor.route(vertxRequest.getRequest(), response, response);
        return vertxRequest;
    }
}
